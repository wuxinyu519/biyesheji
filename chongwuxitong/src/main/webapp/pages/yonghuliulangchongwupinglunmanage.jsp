					...
					<div class=" col-sm-12">
						<div class="card card-shadow mb-4">
							<div class="card-header">
								<div class="card-title">宠物评论管理</div>
							</div>
							<div class="card-body">
									...
									<tbody>
										...
										<c:if test="${ !empty liulangchongwupinglunall}">
											...
											<c:forEach items="${ liulangchongwupinglunall}" var="item">
												...
												<tr>
													<td>${item.liulangchongwu }</td>
													<td>${item.liulangchongwuid }</td>
													<td>${item.neirong }</td>
													<td>${item.pinglunrenmingzi }</td>
													<td>${item.pinglunrenid }</td>
													<td>${item.pinglunshijian }</td>
													<td>
														<!--  定义修改宠物评论按钮和删除宠物评论按钮，修改提交地址为yonghuupdateliulangchongwupinglun.action?id=${item.id}，删除提交地址为 yonghudeleteliulangchongwupinglun.action?id=${item.id}-->

														<a href="liulangchongwudetails.action?id=${item.liulangchongwuid}">查看</a>
														<a href="yonghudeleteliulangchongwupinglun.action?id=${item.id}">删除</a>
