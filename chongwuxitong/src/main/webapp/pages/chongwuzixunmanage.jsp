					...
					<div class=" col-sm-12">
						<div class="card card-shadow mb-4">
							<div class="card-header">
								<div class="card-title">宠物资讯管理</div>
							</div>
							<div class="card-body">
								<table id="bs4-table" class="table table-bordered table-striped"
									...
									<tbody>
										...
										<c:if test="${ !empty chongwuzixunall}">

											<c:forEach items="${ chongwuzixunall}" var="item">



												<tr>
													<td>${item.biaoti }</td>
													<td>${item.fabushijian }</td>
													<td>${item.neirong }</td>
													<td>
														<!-- 将图片从服务器中获取并使用img展示，设置其宽高为100px --> <a
														href="resource/uploads/${item.fengmian }"><img
															src="resource/uploads/${item.fengmian }"
															style="height: 100px; width: 100px;" /></a>

													</td>
													<td>
														<!--  定义修改宠物资讯按钮和删除宠物资讯按钮，修改提交地址为updatechongwuzixun.action?id=${item.id}，删除提交地址为 deletechongwuzixun.action?id=${item.id}-->

														<a href="updatechongwuzixun.action?id=${item.id}">修改</a> <a
														href="deletechongwuzixun.action?id=${item.id}">删除</a>
