...
<body>
	<div id="wrapper">
		...
		<section class="hero-sec bg-full"
			...
			<div class="caption">
				<h1 class="heading text-center">流浪宠物详情</h1>
			</div>
		</section>
		<section class="abt-sec container">
			<div class="row">
				<div class="col-xs-12 col-sm-12" style="margin-bottom: 80px;">
					<header class="header text-center">
						<h2 class="heading text-uppercase">${liulangchongwu.mingzi}</h2>
					</header>

					<!--  使用img展示流浪宠物的图片  -->
					<img src="resource/uploads/${liulangchongwu.tupian}"
					...
					<div>
						<span
							style="display: inline-block; width: 113px; text-align: right; margin-right: 20px;"><b>介绍：</b></span>${liulangchongwu.jieshao}</div>


					<!--  展示流浪宠物的注意事项  -->
					<div>
						<span
							style="display: inline-block; width: 113px; text-align: right; margin-right: 20px;"><b>注意事项：</b></span>${liulangchongwu.zhuyi}</div>


					<!--  展示流浪宠物的类别  -->
					<div>
						<span
							style="display: inline-block; width: 113px; text-align: right; margin-right: 20px;"><b>类别：</b></span>${liulangchongwu.leibie}</div>


					<!--  展示流浪宠物的状态  -->
					<div>
						<span
							style="display: inline-block; width: 113px; text-align: right; margin-right: 20px;"><b>状态：</b></span>${liulangchongwu.state}</div>
			...
			<div class="container">
				<div id="caozuomoban" style="margin-bottom: 40px;">
					<c:if test="${sessionScope.identity == '用户' && liulangchongwu.state == '未领养'}">
						<a class="btn btn-primary waves-effect waves-light" href="yonghuaddlingyangact.action?liulangchongwuid=${liulangchongwu.id}">领养</a>
					</c:if>
				</div>
				<script>
				...
				<div style="margin-bottom: 80px;">
					<div>
						...
						<!-- 判断{表名备注}评论内容是否为空  -->
						<c:if test="${ !empty liulangchongwupinglunall}">
							<!-- 循环展示{表名备注}评论内容  -->
							<c:forEach items="${ liulangchongwupinglunall}" var="item">
...
document.getElementById("dangqianshijian").value = new Date().toLocaleString();
								...
								<input readonly type="text" id="dangqianshijian"
									...
									type="hidden" value="${liulangchongwu.mingzi }"
									name="liulangchongwu" class="form-control"> <input
									type="hidden" value="${liulangchongwu.id }"
									name="liulangchongwuid" class="form-control"> <input
									type="hidden" value="${sessionScope.mingzi }"
									name="pinglunrenmingzi" class="form-control"> <input
									type="hidden" value="${sessionScope.id }" name="pinglunrenid"
									class="form-control"> <input type="hidden"
									value="liulangchongwudetails.action?id=${liulangchongwu.id }"
									name="backurl" class="form-control">
							</div>
