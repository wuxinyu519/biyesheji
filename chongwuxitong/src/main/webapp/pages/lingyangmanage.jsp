							...
							<div class="card-body">
								<table id="bs4-table" class="table table-bordered table-striped"
									...
									<tbody>
										...
										<c:if test="${ !empty lingyangall}">

											<c:forEach items="${ lingyangall}" var="item">
												...
												<tr>
													...
													<td>${item.lyriqi }</td>
													<td>
														<!--  定义修改领养按钮和删除领养按钮，修改提交地址为updatelingyang.action?id=${item.id}，删除提交地址为 deletelingyang.action?id=${item.id}-->

														<a href="updatelingyang.action?id=${item.id}">修改</a> <a
														href="deletelingyang.action?id=${item.id}">删除</a>
