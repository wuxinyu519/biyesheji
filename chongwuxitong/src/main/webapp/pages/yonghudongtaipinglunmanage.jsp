										...
										<c:if test="${ !empty dongtaipinglunall}">
											...
											<c:forEach items="${ dongtaipinglunall}" var="item">
												...
												<tr>
													...
													<td>
														<!--  定义修改动态回复按钮和删除动态回复按钮，修改提交地址为yonghuupdatedongtaipinglun.action?id=${item.id}，删除提交地址为 yonghudeletedongtaipinglun.action?id=${item.id}-->

														<a href="dongtaidetails.action?id=${item.dongtaiid}">查看</a>
														<a href="yonghudeletedongtaipinglun.action?id=${item.id}">删除</a>
