					...
					<div class=" col-sm-12">
						<div class="card card-shadow mb-4">
							<div class="card-header">
								<div class="card-title">动态回复管理</div>
							</div>
							<div class="card-body">
									...
									<tbody>
										...
										<c:if test="${ !empty dongtaipinglunall}">
											...
											<c:forEach items="${ dongtaipinglunall}" var="item">
												...
												<tr>
													<td>${item.dongtai }</td>
													<td>${item.dongtaiid }</td>
													<td>${item.neirong }</td>
													<td>${item.pinglunrenmingzi }</td>
													<td>${item.pinglunrenid }</td>
													<td>${item.pinglunshijian }</td>
													<td>
														<!--  定义修改动态回复按钮和删除动态回复按钮，修改提交地址为updatedongtaipinglun.action?id=${item.id}，删除提交地址为 deletedongtaipinglun.action?id=${item.id}-->

														<a href="updatedongtaipinglun.action?id=${item.id}">修改</a>
														<a href="deletedongtaipinglun.action?id=${item.id}">删除</a>
